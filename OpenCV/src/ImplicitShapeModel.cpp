...
//	std::cerr << _std_size << "/" << shape_location.size << std::endl;
	...
	for(int i = 0; i < features.rows; i++){
		...
		for(int w = 0; w < _vocaburary.cols;w++){
			...
			psn = sample_num.find(entry.class_response());
			if(sample_num.end()==psn){
				sample_num[entry.class_response()] = std::vector<size_t>(wordNum(),0);
				psn = sample_num.find(entry.class_response());
			}
			psn->second[w]++;
...
//	float kernel_size = sqrt(location.size);
	...
	size_t class_num = 0;
	for(std::map<int,std::vector<size_t> >::const_iterator pp=sample_num.begin();
			pp!=sample_num.end();pp++,class_num++){
	...
	for(int i = 0; i < features.rows; i++){
		...
		for(size_t ci=0;ci<class_num;ci++){
			...
			for(size_t w=0; w<wordNum(); w++){
				float occ_weight = 1.f / sample_num.find(class_response)->second[w];
