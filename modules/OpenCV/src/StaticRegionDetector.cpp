...
size_t StaticRegionDetector::compute(
	...
	if(object_labels.size()!=region_labels.size()){
		object_labels = cv::Mat::zeros(region_labels.size(),CV_16SC1);
	}
	else{
	...
	std::vector<size_t> id2static_object_id(region_num,0),id2dynamic_object_id(region_num,0);
	size_t sid(1),did(1);
	...
	for(int y=0;y<region_labels.rows;y++){
		for(int x=0;x<region_labels.cols;x++){
			...
			if(is_static_object[id]){
				object_labels.at<short>(y,x) = id2static_object_id[id];
			}
			else{
				prev_labels.at<short>(y,x) = id2dynamic_object_id[id];
			}
		}
	}
#ifdef DEBUG
