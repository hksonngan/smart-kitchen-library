...
void Patch::set(const cv::Mat& __mask, const cv::Mat& img, const cv::Mat& current_bg, const cv::Mat& fg_edge, const cv::Rect& roi){
	...
	cv::blur(_mask[dilate],_mask[dilate],cv::Size(PATCH_DILATE,PATCH_DILATE));
	cv::threshold(_mask[dilate],_mask[dilate],0.0f,1.0f,CV_THRESH_BINARY);
	cv::blur(_mask[dilate],_mask[dilate],cv::Size(PATCH_DILATE,PATCH_DILATE));
	...
	_edge = cv::Mat(fg_edge,roi);
	_edge &= _mask[original];
