...
#define TAKEN_OBJECT_EDGE_CORELATION 0.6
...
#define PATCH_DILATE 32
	...
	class PatchModel{
		...
		public:
			...
			void update(const cv::Mat& newest_img, const cv::Mat& object_mask, float learning_rate);
			...
			const std::list<size_t>& hidden_objects()const{return _hidden_objects;}
			const std::list<size_t>& newly_hidden_objects()const{return _newly_hidden_objects;}
			const std::list<size_t>& reappeared_objects()const{return _reappeared_objects;}
		...
		protected:
			...
			bool checkTakenObject(size_t id, const cv::Mat& bg_edge)const;
