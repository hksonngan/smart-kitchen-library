	...
	namespace gpu{
		...
		class TexCut: public BackgroundSubtractAlgorithm{
			public:
				...
				void updateBackgroundModel(const cv::gpu::GpuMat& img){setBackground(img);}
				...
				void setBackground(const cv::Mat& bg){setBackground(cv::gpu::GpuMat(bg));}
				void learnImageNoiseModel(const cv::Mat& bg2){learnImageNoiseModel(cv::gpu::GpuMat(bg2));}

				void updateBackgroundModel(const cv::Mat& img){updateBackgroundModel(cv::gpu::GpuMat(img));}
